 ...
 * Config: Similar to Claude/Cursor
...
export class AntigravityProvider implements Provider {
  ...
  configPath = (home: string) => {
    const os = platform();
    if (os === 'darwin') {
      return join(home, 'Library', 'Application Support', 'Antigravity', 'settings.json');
    } else {
  ...
  transformMcpServers(servers: Record<string, McpServer>): Record<string, unknown> {
    ...
    for (const [name, server] of Object.entries(servers)) {
      if (server.url) {
        console.warn(`Skipping HTTP server "${name}" - Antigravity only supports stdio servers`);
        continue;
      }
