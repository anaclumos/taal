...
describe('ClaudeDesktopProvider', () => {
  ...
  it('should resolve config path correctly', () => {
    ...
    const path = typeof provider.configPath === 'function' 
      ? provider.configPath(home) 
      : provider.configPath;
    
    expect(path).toBe('/Users/testuser/Library/Application Support/Claude/claude_desktop_config.json');
  });
  
  it('should resolve skills path correctly', () => {
    const home = '/Users/testuser';
    const path = typeof provider.skillsPath === 'function' 
  ...
  it('should read config from file', async () => {
    ...
    const configDir = join(testHome, 'Library', 'Application Support', 'Claude');
