...
import { copySkillsToProvider, copySkill } from '../../src/skills/copy.js';
...
`);
    ...
    const skills: Skill[] = [
      { name: 'Skill 1', path: skill1Dir, skillMdPath: join(skill1Dir, 'SKILL.md') },
      { name: 'Skill 2', path: skill2Dir, skillMdPath: join(skill2Dir, 'SKILL.md') },
    ];
...
`);
    ...
    const skills: Skill[] = [
      { name: 'Test Skill', path: skillDir, skillMdPath: join(skillDir, 'SKILL.md') },
    ];
...
`);
    ...
    const skills: Skill[] = [
      { name: 'Complex Skill', path: skillDir, skillMdPath: join(skillDir, 'SKILL.md') },
    ];
    ...
    expect(existsSync(join(targetDir, 'complex-skill', 'scripts', 'helper.sh'))).toBe(true);
    expect(existsSync(join(targetDir, 'complex-skill', 'references', 'doc.md'))).toBe(true);
  });
...
`);
    ...
    const content = readFileSync(join(targetDir, 'test-skill', 'SKILL.md'), 'utf-8');
